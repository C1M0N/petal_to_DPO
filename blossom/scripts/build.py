...
# =========================================================
# 注意：花书拆解/重要章节/ 没有第 1 章的朱明超精读版（从第 2 章起才有）。
# 本章的「📖 观点提要」是 blossom 按 Goodfellow et al. *Deep Learning*
# Chapter 1 (Introduction) 的公开内容结构自行整理转述——不是原文引用。
# 结构跟原书第 1 章走：
#   §0 深度学习是什么（AI ⊃ ML ⊃ 表示学习 ⊃ DL）
#   §1 表示的力量（笛卡尔 vs 极坐标实验，对应原书图 1.1）
#   §2 深度 = 复合函数（两种深度度量）
#   §3 三次浪潮（原书 §1.2.1）
#   §4 与日俱增的数据量（原书 §1.2.2）+ sklearn digits 初体验
#   §5 与日俱增的模型规模（原书 §1.2.3）
#   §6 与日俱增的精度与影响（原书 §1.2.4）
# 趋势图数据硬编码自原书图 1.8 / 1.10 / 1.11 的公开数值（均为近似值）。
# =========================================================


def ch01_header(b: Builder):
    b.md(
        "# 花书 · 第一章：引言\n\n"
        "> 你打开的是 demo / work 副本。**学习时用 work 副本**——\n"
...
def ch01_p0_landscape(b: Builder):
    b.md("---\n\n## §0 深度学习是什么：AI ⊃ ML ⊃ 表示学习 ⊃ DL\n")
    b.md(
    ...
    b.code(
        ...
        "category = {\n"
        "    'Cyc 知识库（手写逻辑规则做推理）':            'AI',\n"
        "    'logistic regression（用人工设计的特征）':     'ML',\n"
        "    '浅层 autoencoder（自动学习一层特征）':        '表示学习',\n"
        "    '多层感知机 MLP（逐层学习嵌套表示）':          'DL',\n"
        "}\n"
    ...
    b.code(
        ...
        "chain_holds = DL <= REP <= ML <= AI          # 子集判断可以像比较一样连起来写\n"
...
def ch01_p1_representation(b: Builder):
    ...
    b.code(
        "pred_polar = (r > 1.5).astype(int)\n"
        "acc_polar = (pred_polar == label).mean()\n"
...
def ch01_p2_depth(b: Builder):
    ...
    b.code(
        ...
        "f2 = lambda x: x + 2\n"
        "f3 = lambda x: x**2\n\n"
        "g = lambda x: f3(f2(f1(x)))          # 深度 3 的复合\n"
        "depth = 3\n\n"
        "print('g(1) =', g(1))\n"
        "print('g(0) =', g(0))\n\n"
...
def ch01_p3_waves(b: Builder):
    ...
    b.code(
        "wave = {\n"
        "    '感知机 perceptron (1958)':                    1,\n"
        "    'Minsky & Papert 指出线性模型学不了 XOR (1969)': 1,\n"
        "    '反向传播训练多层网络普及 (1986)':               2,\n"
        "    'LSTM 提出 (1997)':                            2,\n"
        "    'deep belief network 逐层预训练 (2006)':        3,\n"
        "    'AlexNet 在 ImageNet 夺冠 (2012)':              3,\n"
        "}\n"
...
def ch01_p4_data(b: Builder):
    ...
    b.code(
        ...
        "ax.set_yscale('log')                 # ← 对数坐标：每格 = 一个数量级\n"
    ...
    b.code(
        ...
        "n_pixels = digits.data.shape[1]\n"
        "flat0 = digits.images[0].reshape(-1)\n\n"
    ...
    b.code(
        "px_min = digits.data.min()\n"
        "px_max = digits.data.max()\n"
        "print(f'像素范围：[{px_min}, {px_max}]')   # 猜对了吗？digits 是 17 级灰度，不是 256 级\n\n"
...
def ch01_p5_model_size(b: Builder):
    b.md("---\n\n## §5 与日俱增的模型规模（原书 §1.2.3）\n")
    b.md(
        "#### 📖 花书第 1 章 · 观点提要（blossom 转述，非原文）\n\n"
    ...
    b.code(
        "n_doublings = np.log2(8.6e10 / 1e6)\n"
        "years_needed = n_doublings * 2.4\n"
...
def ch01_p6_impact(b: Builder):
    ...
    b.code(
        ...
        "rel_drop = (top5_err[0] - top5_err[-1]) / top5_err[0]\n"
    ...
    b.code(
        ...
        "year_biggest = ilsvrc_years[np.argmin(drops) + 1]\n"
...
def build_ch01():
    ...
    ch01_header(b)
    ch01_p0_landscape(b)
    ch01_p1_representation(b)
    ch01_p2_depth(b)
    ch01_p3_waves(b)
    ch01_p4_data(b)
    ch01_p5_model_size(b)
    ch01_p6_impact(b)
    ch01_checkpoint(b)
    write_ipynb(b.demo, "ch01-introduction/ch01.ipynb")
    write_ipynb(b.work, "ch01-introduction/ch01_work.ipynb")
